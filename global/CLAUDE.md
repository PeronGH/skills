...
- NEVER use the Agent tool. Subagent results are summaries that lose detail. Do all work directly.
...
- Code is the primary source of truth. Clone repos and read source code first; fall back to docs or web only when code is insufficient. NEVER fetch code via URL.
