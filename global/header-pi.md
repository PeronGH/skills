...
- Browse the web frequently. Use the `web` CLI to check anything you're unsure of or that may be outdated. Run `web --help` first, then use `web search "$QUERY"` and `web fetch "$URL"`; snippets are too short, so fetch full text.
